...
describe("desktop renderer UI integration", () => {
  ...
  test("wires changed files to the bounded workspace diff modal", async () => {
    ...
    expect(app).toContain("window.easycode.workspaceDiff(filePath, workspaceRoot, scope)")
    expect(app).toContain("<WorkspaceDiffModal")
    expect(app).toContain("workspaceDiffNavigation(workspaceChangePaths(workspaceStatus?.files ?? [], activeWorkspaceDiffScope)")
    ...
    expect(workspaceDiff).toContain('scope === "staged"')
  ...
  test("refreshes workspace status and the active diff through existing desktop APIs", async () => {
    ...
    expect(app).toContain("refreshWorkspaceDiff(filePath, scope)")
